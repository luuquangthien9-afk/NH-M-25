...
# Biến toàn cục
reset_game()
das_delay = 0.16  # Sửa từ 0.15 thành 0.16
arr_speed = 0.03  # Sửa từ 0.05 thành 0.03
...
while running:
    clock.tick(60)
    fall_time += clock.get_time() / 1000.0  # Sửa từ get_delta() thành get_time()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not paused and not game_over and not confirm_quit:
                if event.key == pygame.K_1:  # Thêm K_1 cho pause (từ xephinh.py)
                    paused = not paused
                if event.key == pygame.K_LSHIFT or event.key == pygame.K_RSHIFT:
                    paused = True

                if event.key == pygame.K_DOWN:
                    fall_speed = 0.02  # Sửa soft drop từ 0.05 thành 0.02
    ...
    if in_start_screen:
        ...
        if fall_time >= fall_speed:
            ...
            if valid_move(current_piece, create_grid(locked), dy=1):
                ...
                fall_speed = get_fall_speed(score)
                current_level = 1 + (score // 500)  # Sửa từ //1000 thành //500
